...
// $_GET can execute even Javascript script by echoing <script></script>
// So it should be replaced into htmlspecialchars like below
// not echo $_GET['name'] ?? '';
// echo htmlspecialchars($_GET['name']);

function filterListingsByLocation($jobs, $location)
{
  return array_filter($jobs, function ($job) use ($location) {
...
if (isset($_GET['location']) && in_array($_GET['location'], $validLocations)) {
  ...
  $filteredListings = filterListingsByLocation($jobs, $location);
} else {
  $filteredListings = $jobs;
}
?>
...
<body class="bg-gray-100">
  ...
  <div class="container mx-auto p-4 mt-4">
    <div class="bg-green-100 rounded-lg shadow-md p-6 my-6">
      <h2 class="text-2xl font-semibold mb-4">Average Salary: <?= calculateAverageSalary($jobs)  ?></h2>
    </div>
    <!-- Output -->
